          ...
          <form action="DetermineIslamicActivity.php" method ="post" class="donation_form">
            <div style="margin-left:2%; margin-top:5%;padding:5%">
              ...
              <table id="customers">
                ...
                $sql = "SELECT IslamicVolunteerID, lessonName, meetingNumber from islamicvolunteer";
                $result = $connect-> query($sql);
                if ($result-> num_rows > 0){
                  while ($row = $result-> fetch_assoc()){

                    echo "<tr><td>". $row["meetingNumber"] ."</td><td>" . $row["lessonName"] ."</td><td>" .'<input type="checkbox" name= "checkbox[]" value ='.$row["IslamicVolunteerID"].'>'."</td></tr>";
...
if(isset($_POST['submit'])){
  if(!empty($_POST['checkbox'])){
    foreach($_POST['checkbox'] as $row){
      ...
      $sqll = "SELECT IslamicVolunteerID, meetingNumber, lessonName FROM islamicvolunteer WHERE IslamicVolunteerID = $chosenLesson ";
      ...
      if (mysqli_num_rows($resultt) > 0) {
          ...
          $attendancenummber = $row1["meetingNumber"];
