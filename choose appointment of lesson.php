...
height: 400px;
 ...
 function alert() {
      ...
      window.setTimeout(function() {
        location.replace("choose appointment of lesson.php");
        }, 2020);
...
<li><a style="font-size:15px;">|</a></li>
  ...
  $appo;
        ...
        <tbody id='bb'>
           ...
           $sql1= "SELECT day ,timeStart, timeEnd,volunteerName	 ,	idFollowUpLesson FROM followuplesson  where studentId	= $student And surahName='0' ";
           if ($result1 = $connect-> query($sql1)) {
             if(mysqli_num_rows($result1)>0){
              while ($obj = $result1 -> fetch_object()) {
            $appo =new Appointment($obj->volunteerName,$obj->timeStart,$obj->timeEnd,"0",$obj->day);
              ...
              echo"<td><output style=' padding-left:5px;padding-right:5px; border-bottom: 2px solid #07B5B2;  background-color:#f6f6f6 ;'>"; echo$appo->get_timeStart();echo"</output> </td> ";
               ...
               echo"<td>"; echo$appo->get_volunteer_name();echo"</td>   </tr> </tbody>";
                ...
                if(isset($_POST['delete'])){
                   $sql3 = "DELETE FROM followuplesson WHERE idFollowUpLesson=$obj->idFollowUpLesson ";
...
<?php
echo "<tbody>";
$quranID = $_GET['quranID'];
     $sql= "SELECT facultymember.FacultyMemberName, quranvolunteer.Quran_volunteer_id , quranvolunteer.Surah_name   FROM quranvolunteer INNER JOIN facultymember ON quranvolunteer.FacultyUniversityID = facultymember.FacultyUniversityID where quranvolunteer.requestState='مقبول' AND Quran_volunteer_id=$quranID";
     if ($result = $connect-> query($sql)) {
       ...
       $result -> free_result();
         ...
         echo "</tbody>";
         echo "</table>";
         echo "<input  type='submit'  name ='submit' value='حفظ'  class='boxed-btn4' style=' margin:5% 44%;'>";
         echo "</form>";
             ...
             if ($result1 = $connect-> query($sql4)) {
               while ($obj = $result1 -> fetch_object()) {
                 ...
                 if ($result2 = $connect-> query($sql)) {
                  ...
                  $sql2 = "INSERT INTO followuplesson  (studentId	, day ,timeStart,timeEnd, volunteerName )
                 VALUES ('$studentID',   '$obj->Day','$obj->timeStart', '$obj->timeEnd', '$obj1->FacultyMemberName')";
