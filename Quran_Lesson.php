...
<div class="slider_area" style="padding-left:1%">
    <div class="single_slider  d-flex align-items-center   " style="  background-image: url(Asrar-files/2.png);   background-size:auto;   background-position:top ; ">
      <div class="row text-right" style="margin-left:62%; margin-top:-23%;">
        <div class="col-12 text-right">
          <div class="section_title text-right mb-55">
            <h3 dir="rtl" style="font-size:30px"><span>دروس المصلى > حلقات التقوية      </span></h3>
          </div>
        </div>
      </div>
    </div>
...
function alertError(){
  Swal.fire({
        icon: 'warning',
        title: 'أنت مسجل من قبل في هذا الدرس ',
        showConfirmButton: false,
        timer: 2000
      })
}
...
<script>
    ...
    //Activity-student.php
    var lesson_div = document.getElementById('lesson-div');
    $.ajax({
      url: 'Asrar-files/quran_lesson.php',
      type: 'get',
      dataType: 'JSON',
      success: function(response) {
        ...
        for (var i = 0; i < len; i++) {
          ...
          let title="حلقات التقوية لتلاوة القرآن الكريم";
          let text="سيتم إختيار الموعد المناسب بعد التسجيل من صفحة الدروس المسجلة";
          let code =
            ...
            '<h3 class="text-center" style=" margin-left: auto; margin-right: auto;">' + title + '<br>' +
            FacultyMemberName + '</h3> </div>' +
            '<p dir="rtl">   ' + Surah_name  + '  </p>' +
            '<p dir="rtl">   ' +  text + '  </p>' +
            // '<p dir="rtl"><i class="fa fa-calendar"></i>   ' + Day  + '  </p>' +
            // '<p dir="rtl"><i class="fa fa-clock-o"></i> ' + timeStart +' - ' +timeEnd + '</p>' +
            '<a class="genric-btn success radius" style="color: rgb(255, 255, 255); " onmouseout="this.style.color= `#fff` " onmouseover="this.style.color= `#07B5B2` "  onclick="registerLesson(event)" id=" ' + Quran_volunteer_id + ' ">التسجيل</a></div></div></div>';
  ...
  <script>
    ...
    // register student in activity lesson
    function registerLesson(event) {
      var Activity_ID = event.target.id;
      $.ajax({
