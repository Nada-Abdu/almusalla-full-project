...
<body>
  ...
  <div class="slider_area" style= "padding-left:1%">
    <div class="single_slider  d-flex align-items-center   " style="  background-image: url(image_personal_page/1.png);   background-size:auto;   background-position:top ;">
      ...
      <div class="row text-right" style="margin-left:73%; margin-top:-26%;">
...
<div id="requestForm" style=" width:1100px; height: 300px; overflow-x: hidden; overflow-y: auto; margin-left:9% ;margin-top:-5%">
  <?php
  $StudentsA= mysqli_query($connect,"SELECT * FROM `followupactivity` WHERE `activity_id`='$activityid' group by `student_id` ");
if(mysqli_num_rows($StudentsA) == 0){ ?>

  <label style="color:#07B5B2;width:1010px; border-bottom:none; padding-left:6%;">
    <input readonly type="text"  value="عــــــــــــــذرًا لا يـــــوجـــــد طـــــالبــــات" style="text-align:center; font-size:15px; height:10%;   padding: 15px 25px;"/>
  </label>
...
$choosedactivtyStudent = $_POST['activty'] ?? '';
 $ActivtyStudent= mysqli_query($connect,"SELECT * FROM  `followupactivity` WHERE `student_id`='$choosedactivtyStudent' and `activity_id`='$activityid' group by `student_id`");
                                 ...
                                 <div class="fixed_donat">
                                     <div class="select_prise"  style="padding:1%; padding-right:4% "  >
                                       <div class="row">
                                         ...
                                         <?php
                                         $infoActvties= mysqli_query($connect,"SELECT * FROM `activities` WHERE `activityid`='$activityid'"); // activityid from session or from click
...
<?php
  ...
  $infoStudents= mysqli_query($connect,"SELECT * FROM `followupactivity` WHERE `student_id`='$choosedactivtyStudent' and `activity_id`='$activityid' ");
  ...
  while ($infoStudent= mysqli_fetch_assoc($infoStudents)) {
   ...
   <tr>
     ...
     <td><?php echo $infoStudent['Surah_name']; ?></td>
       <td>
<?php
if($infoStudent['use_Al-mushaf_Al-mualem'] == 1) {
